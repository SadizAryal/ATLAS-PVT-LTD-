...
export default function ManagerPage() {
  ...
  if (!unlocked) return <div className="manager-gate wrap"><p className="eyebrow">Manager view</p><h1>Demo access</h1><p>Enter the access code to view training records.</p><form onSubmit={unlock}><label htmlFor="manager-code">Demo access code</label><input id="manager-code" value={code} onChange={(event) => { setCode(event.target.value); setError('') }} autoComplete="off" aria-describedby="manager-code-hint manager-code-error" aria-invalid={Boolean(error)} /><small id="manager-code-hint">For this demo use COLAB-2026.</small>{error && <p id="manager-code-error" className="gate-error" role="alert">{error}</p>}<button className="button primary" type="submit" aria-label="Unlock manager view">Unlock manager view</button></form></div>
