...
type Viewer = { lookAt: (pitch: number, yaw: number, hfov?: number) => void; getYaw: () => number; getPitch: () => number; getHfov: () => number }
...
export default function PanoViewer({ onOpen, onReady }: Props) {
  ...
  useEffect(() => { onReady?.({ lookAt: (yaw, pitch) => viewer.current?.lookAt(pitch, yaw, viewer.current.getHfov()) }); return () => { viewer.current = null } }, [onReady])
  function init() { const pannellum = (window as Window & { pannellum?: { viewer: (el: HTMLDivElement, opts: Record<string, unknown>) => Viewer } }).pannellum; if (!pannellum || !ref.current) return; try { viewer.current = pannellum.viewer(ref.current, { type: CONFIG.PANO.TYPE, panorama: CONFIG.PANO.SRC, autoLoad: true, showControls: false, draggable: false, keyboardZoom: true, mouseZoom: true, touchPanEnabled: false, friction: 0.15, compass: false, yaw: CONFIG.PANO.INITIAL_YAW, pitch: CONFIG.PANO.INITIAL_PITCH, hfov: CONFIG.PANO.INITIAL_HFOV, hotSpots: CONFIG.HOTSPOTS.map((spot) => ({ pitch: spot.pitch, yaw: spot.yaw, cssClass: 'atlas-hotspot', createTooltipFunc: (hotspot: HTMLElement) => { hotspot.setAttribute('aria-label', spot.label); hotspot.title = spot.label }, clickHandlerFunc: () => onOpen(spot) })) }) } catch { setFailed(true) } }
